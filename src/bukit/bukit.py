...
class Module:
    ...
    def build(self, makefile, name):
        ...
        if name is None:
            ...
            protos = self._protos.get(name) or []
            self._generate(protos)
        ...
        for artifact in artifacts:
            ...
            rules.append((artifact.name(), result))
        self._make(rules, makefile)
        return protos, rules
...
class Storage:
    ...
    def save(self, output, protos, rules):
        cache = {}
...
class Bukit:
    ...
    def _build(self, name=None, optimize=None):
        ...
        protos, rules = module.build("Makefile", name)
        with Storage("c") as s:
            s.save(module.output(), protos, rules)
